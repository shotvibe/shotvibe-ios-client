...
//        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectInset(self.bounds, 0.0f, 0.0f)];
    ...
    for (NSInteger i = 0; i < 3; i++) {
        ...
        if (self.showsGridMajor) {
            if (i > 0) {
                ...
                UIRectFill(CGRectMake(roundf(width / 3 * i), borderPadding, 0.5f, roundf(height) - borderPadding * 2));
                UIRectFill(CGRectMake(borderPadding, roundf(height / 3 * i), roundf(width) - borderPadding * 2, 0.5f));
            }
        }
    }
}
