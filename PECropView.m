...
//    self.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    self.imageV = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height*0.75)];
...
{
    self.cropRectView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height*0.75);
...
//    self.imageView.layer
    ...
    if(self.image.size.width > self.image.size.height){//landscape
        self.maskLayer.shadowPath = CGPathCreateWithRoundedRect(CGRectInset(self.imageView.bounds, -10, 10), 10, 10, nil);
    } else {
        self.maskLayer.shadowPath = CGPathCreateWithRoundedRect(CGRectInset(self.imageView.bounds, 10, -10), 10, 10, nil);
    }
    ...
    self.maskLayer.shadowOffset = CGSizeMake(0, 1);
    self.maskLayer.shadowColor = [UIColor whiteColor].CGColor;
    
    self.imageView.layer.mask = self.maskLayer;
    
    
    
    self.imageV.image = [self.image applyBlurWithRadius:30 tintColor:[UIColor colorWithWhite:0.6 alpha:0.2] saturationDeltaFactor:1.0 maskImage:self.image];
//    self.imageV.image = [self maskImage:<#(UIImage *)#> withMask:<#(UIImage *)#>]
